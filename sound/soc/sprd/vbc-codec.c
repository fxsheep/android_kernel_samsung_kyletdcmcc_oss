...
void vbc_power_on(unsigned int value)
{
	...
	pr_debug("audio %s\n", __func__);
	{
		...
		//	pr_info("peter want to delay another 100ms\n");
			msleep(150);
...
#if VBC_NOSIE_CURRENT_SOUND_HARDWARE_BUG_FIX
			if ((!dac_muted || !!vbc_reg_read(VBCR1, DAC_MUTE, 1)) && \
...
{	
	...
	int16_t arm_vol = 0;
	int16_t fm_temp = 0;
	unsigned int device_mode = 0;
	uint32_t i;
	struct s_nv { /* use static variable to avoid large stack use */
		int volume_type;
		int volume_level;
		int eq_type;
		#define S_TUNE_FM_HEADSET	(1 << 0)
		#define S_TUNE_FM_SPEAKER	(1 << 1)
		int fm_type;
		unsigned int device_mode ; //jinwon.baek 120824
		AUDIO_TOTAL_T item;
	} *nv;
	if (count != sizeof *nv) {
		pr_err("kclass_vbc_param_store get wrong data size <%d,%d>", sizeof *nv, count);
		return 0;
	}
	nv = (struct s_nv*)buf;
	vol_type = nv->volume_type;
	vol_index = nv->volume_level;
	eq_type = nv->eq_type;
	fm_type = nv->fm_type;
	audio_param_ptr = &nv->item;
	device_mode = nv->device_mode;
	static unsigned int pre_device_mode = -1;
...
//	sprd_local_audio_pa_mode = audio_param_ptr->audio_nv_arm_mode_info.tAudioNvArmModeStruct.reserve[AUDIO_NV_INTPA_SWITCH_INDEX];
	...
	cur_fm_gain_l = (audio_param_ptr->audio_nv_arm_mode_info.tAudioNvArmModeStruct.reserve[AUDIO_NV_FM_GAINL_INDEX+vol_index]>>8) & 0x001F;	//GOBL
	cur_fm_gain_r = cur_fm_gain_l;	//GOBR
	printk("kclass_vbc_param_store AUDENHA_SetPara pre_device_mode=[%d], device_mode=[%d]\n",pre_device_mode,device_mode);
	if(pre_device_mode != device_mode &&device_mode!=1 && pre_device_mode!=1)
	{//jinwon.baek 120824
		printk("kclass_vbc_param_store AUDENHA_SetPara working!\n");
	...
	{
		printk("kclass_vbc_param_store AUDENHA_SetPara is not working!!\n");
	}
	if(device_mode!=4) 
	{
	pre_device_mode = device_mode;
...
//	printk("kclass_vbc_param_store AUDENHA_SetPara pre_device_mode = [%d] device_mode = [%d] \n" ,pre_device_mode, device_mode);
	...
	arm_vol=audio_param_ptr->audio_nv_arm_mode_info.tAudioNvArmModeStruct.app_config_info_set.app_config_info[vol_type].arm_volume[vol_index];
/*	if((fm_type & S_TUNE_FM_HEADSET) || b_fm_headset_on){
		fm_temp = arm_vol&0xfe0f;
		arm_vol = (((audio_param_ptr->audio_nv_arm_mode_info.tAudioNvArmModeStruct.reserve[AUDIO_NV_FM_GAINL_INDEX+vol_index]&0x00FF)<<4)&0x1F0)|fm_temp;
	}*/
//	printk("kclass_vbc_param_store have store!arm_vol =[0x%x]\n",arm_vol );
	AUDDEV_SetPGA(arm_vol);
//	AUDDEV_SetPGA(0,arm_vol);
	
	if((fm_type & S_TUNE_FM_SPEAKER) || b_fm_handsfree_on){
